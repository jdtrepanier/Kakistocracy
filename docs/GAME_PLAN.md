...
- **Units.** The three currently-switchable officials (`data/battleRosters.ts`'s `US_BATTLE_UNITS`) fight as a trio, each with their own move/range/power/composure, regardless of who's "active" in the room. "Composure" is combat HP — reaching 0 knocks a unit out of the fight (a *headline*, not a casualty, per §14's tone rules). Every character's full profile — display name/sprite (§8) *and* these combat stats/quirks — is a hand-editable JSON file under `data/characters/` (one per side: `us.json`, one per country), the same treatment the battlegrounds above got, merged into one file per character rather than two systems kept in sync by hand (user: *"Each character's information should also be in a json file"*) — see `data/characters/README.md`.
...
- **Battlefields are getting real terrain, one country at a time.** The plain flat grid above (`data/battlefield.ts`) is still what every battle is actually simulated on, but a country can now dress it with real textures and decorations instead of flat colored tiles — a shared, reusable tile atlas (`public/assets/battle-tiles/`: grass, rocky ground, stone paths, water, cliffs, plus trees/buildings/fences/signs) extracted from user-supplied reference art, layered on via `data/battlegrounds.ts`. Canada is the first: a river-and-bridge border crossing (echoing a reference scene set at a Niagara-Falls-style crossing), with its own custom obstacle layout rather than just the shared field re-skinned. The river deliberately never blocks the actual fighting rows — every unit in this game only attacks at melee range, so a real chokepoint there would decide battles by bottleneck rather than by the roster stats it's supposed to come down to (confirmed by re-running the tactical-battle simulator, §7.1's own balance tool, after an initial draft nearly broke Canada's win rate the same way the original stat bug once did — see `CLAUDE.md`). The terrain art itself went through a couple of rounds once the user could finally see it in-game: an initial crop of the reference sheet baked a dark triangular artifact into every tile (fixed with a precise re-crop), then the tiles were fully replaced with a set generated by another AI from a written spec (`tile-generation-spec.md`) once the user asked for one so they could produce more art independently. Both banks of Canada's river now read as a paved camp/plaza flanking the bridge, matching the reference scene's staging areas, rather than one grass bank and one dirt bank with only the crossing itself paved. The river/bank split itself also had to be reworked once seen in-game: it originally ran along a plain grid column, which reads as a clean vertical line on paper but — because the isometric camera's screen-horizontal position depends on `x - y`, not `x` alone — actually rendered as two disconnected diagonal slivers instead of one crossing, with the US and Canadian camps never reading as clean left/right blocks. Splitting along that same `x - y` diagonal instead fixed it, verified by literally running the camera's own projection formula on both versions rather than eyeballing it (see `CLAUDE.md` for the numbers). Every other country still fights on the plain unstyled field until it gets the same treatment.

The battle screen also picked up its own zoom in/out, separate from the whole-game fit-to-window scale — mainly so a much bigger field like Canada's can be zoomed out to see the whole thing at once instead of only ever seeing whatever slice the camera happens to be panned/centered on.

- **Every country's battleground layout is a hand-editable JSON file, not TypeScript.** Once "change one cell of Canada's map" kept meaning "ask Claude to edit code," Canada's whole layout (grid, terrain, decorations, and its own starting positions for both sides) moved into `data/battlegrounds/canada.json`, with `data/battlegrounds.ts` now just a loader that validates each file's shape and throws a specific, readable error on a bad hand-edit (a non-rectangular grid, a misspelled terrain name, a spawn point off the map, naming the country) instead of silently rendering something broken. `data/battlegrounds/README.md` documents the full schema for a non-programmer editor, including why the river has to be drawn as a diagonal band rather than a straight column (the same `x - y` isometric-projection reasoning noted above) and a warning to re-run the battle simulator if the open fighting rows ever get narrowed. This later generalized to all seven war targets (user: *"you forgot the other json files"*) — the other six countries had been quietly sharing one plain field and one shared pair of spawn arrays; each now has its own independent `data/battlegrounds/<country>.json`, generated as a lossless dump of that old shared data so no existing battle changed, just became separately editable. A future country's custom battleground follows the same pattern — a new JSON file plus one more branch in `getBattleground` — not a new one-off TypeScript file.
- **Grid size is per-file, not fixed at 20×14 globally.** User: *"I also want to edit the GRID_W and GRID_H."* `parseGrid` now derives a grid's width/height from its own shape (rectangular only, no fixed-size check) rather than validating against a shared constant — every other place that renders or simulates a battle (movement/combat engine, isometric projection, battle camera) already derived its bounds from the actual grid array, so this was a scoped change to the loader alone. All seven files still happen to be 20×14 today; nothing stops one from being a different size now.
- **`mapeditor/` (repo root) is a small standalone visual editor for these JSON files** — plain HTML/CSS/JS, no build step, deliberately kept outside the game's own Vite project (user: *"you can put the code outside the game like in the mapeditor folder"*). A plain top-down grid (not the isometric camera) with paintable Grid/Terrain/Props layers and click-to-toggle US/enemy spawn points, live soft validation, and three load paths plus two save paths (a plain download always works; Chrome/Edge's File System Access API can open-and-save-in-place). Its own `parseGrid`/`parsePositions`/`parseProps` are a hand-written port of `data/battlegrounds.ts`'s validators, not a shared import, since the whole point is having no dependency on the game's own toolchain — see `mapeditor/README.md`. It also has a zoom control (bigger/smaller cells for easier clicking) and a width/height resize control (crops/pads the current map, dropping anything that falls outside the new bounds), both purely editor-side conveniences layered on top of the same JSON schema above.
